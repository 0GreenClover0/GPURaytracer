...
inline void ThrowIfFalse(bool value, wchar_t const* msg)
{
...
inline Microsoft::WRL::ComPtr<ID3DBlob> CompileShader(std::wstring const& filename, const D3D_SHADER_MACRO* defines,
                                                      std::string const& entrypoint, std::string const& target)
{
    UINT compileFlags = 0;
#if defined(_DEBUG) || defined(DBG)
...
inline void print_state_object_desc(const D3D12_STATE_OBJECT_DESC* desc)
{
    std::wstringstream wstr;
    wstr << L"\n";
    wstr << L"--------------------------------------------------------------------\n";
