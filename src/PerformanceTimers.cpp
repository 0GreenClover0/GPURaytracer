...
CPUTimer::CPUTimer() : m_cpuFreqInv(1.f), m_start {}, m_end {}, m_avg {}
{
...
void GPUTimer::RestoreDevice(_In_ ID3D12Device* device, _In_ ID3D12CommandQueue* commandQueue, UINT maxFrameCount)
{
    ...
    if (SUCCEEDED(device->QueryInterface(IID_PPV_ARGS(&d3dInfoQueue))))
    {
        ...
        OutputDebugStringA("Warning: GPUTimer is disabling an unwanted D3D12 debug layer warning: "
                           "D3D12_MESSAGE_ID_EXECUTECOMMANDLISTS_GPU_WRITTEN_READBACK_RESOURCE_MAPPED.");
    }
    ...
    ThrowIfFailed(device->CreateCommittedResource(&readBack, D3D12_HEAP_FLAG_NONE, &bufferDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
